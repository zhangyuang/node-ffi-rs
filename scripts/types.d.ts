...
export function load<T extends DataType, U extends Record<string, DataFieldType<T>>>(params: FfiParams<T> & {
  retType: U
}): { [K in keyof U]: DataFieldTypeToType<U[K]> }

export function load<T extends DataType>(params: FfiParams<T> & {
  retType: T
}): DataTypeToType<T>
