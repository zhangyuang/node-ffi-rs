...
static mut LIBRARY_MAP: Option<
    HashMap<String, (Library, HashMap<String, Result<Symbol<unsafe extern "C" fn()>>>)>
> = None;
...
unsafe fn restore_pointer(env: Env, params: StorePointerParams) -> Result<Vec<JsUnknown>> {
    let StorePointerParams { ret_type, params_value } = params;
...
unsafe fn unwrap_pointer(env: Env, params: Vec<JsExternal>) -> Result<Vec<JsExternal>> {
    params
        ...
        .map(|js_external| {
            ...
            env.create_external(internal_ptr, Some(std::mem::size_of::<*mut c_void>() as i64))
        })
        .collect()
}
#[napi]
                ...
                Err(e) =>
                    match e {
                        dlopen::Error::OpeningLibraryError(e) => {
                            ...
                            if
                                err_msg.contains("is not a valid Win32 application") ||
...
unsafe fn get_symbol<'a>(
    ...
    let func = func_map
        ...
        .or_insert_with(|| {
            lib.symbol::<unsafe extern "C" fn()>(&func_name).map_err(|_| {
                FFIError::FunctionNotFound(
                    format!("Cannot find {:?} function in shared library", &func_name)
                ).into()
            })
        })
        .as_ref()
    ...
    if run_in_new_thread == Some(true) {
        ...
        impl Task for FFICALL {
            ...
            fn compute(&mut self) -> Result<BarePointerWrap> {
                let FFICALLPARAMS { mut cif, fn_pointer, .. } = self.data;

                let FFICALLPARAMS { arg_values_c_void, .. } = &mut self.data;
                unsafe {
                    let result = libc::malloc(std::mem::size_of::<*mut c_void>());
                    ffi_call(&mut cif, Some(fn_pointer), result, arg_values_c_void.as_mut_ptr());
                    Ok(BarePointerWrap(result))
                }
            }

            fn resolve(&mut self, env: Env, output: Self::Output) -> Result<JsUnknown> {
                let FFICALLPARAMS { errno, free_result_memory, .. } = self.data;
                let FFICALLPARAMS { ret_type_rs, arg_values_c_void, params_type_rs, .. } =
                    &mut self.data;
                unsafe {
