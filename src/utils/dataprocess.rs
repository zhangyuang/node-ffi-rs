...
pub unsafe fn get_arg_values(
    ...
    if params_type.len() != params_value.len() {
        return Err(
            FFIError::Panic(
                format!("params_type length is not equal with params_value length")
            ).into()
        );
    }
    params_type
            ...
            let res = match param {
                RsArgsValue::I32(number) => {
                    ...
                    match param_data_type {
                        BasicDataType::U8 => {
                            let arg_val: u32 =
                                create_js_value_unchecked::<JsNumber>(value)?.try_into()?;
                        ...
                        BasicDataType::BigInt => {
                            let arg_val: i64 =
                                create_js_value_unchecked::<JsBigInt>(value)?.try_into()?;
                            RsArgsValue::I64(arg_val)
                        }
                        BasicDataType::U64 => {
                            let arg_val: i64 =
                        ...
                        BasicDataType::String => {
                            let arg_val: String = js_string_to_string(
                                create_js_value_unchecked::<JsString>(value)?
                            )?;
                        ...
                        BasicDataType::Boolean => {
                            let arg_val: bool =
                                create_js_value_unchecked::<JsBoolean>(value)?.get_value()?;
                ...
                RsArgsValue::Object(params_type_object_rs) => {
                    if
                        let FFITypeTag::Array | FFITypeTag::StackArray = get_ffi_tag(
                            &params_type_object_rs
                        )
                    {
                        let array_desc = get_array_desc(&params_type_object_rs);
                        let FFIARRARYDESC { array_type, struct_item_type, .. } = array_desc;
                        match array_type {
                            RefDataType::U8Array => {
                            ...
                            RefDataType::StructArray => {
                                ...
                                let arg_values = vec![0; js_object.get_array_length()? as usize]
                                    ...
                                    .enumerate()
                                    .map(|(index, _)| {
                                        let js_element: JsObject = js_object
                                            .get_element(index as u32)
                                            .unwrap();
                                        let struct_item_type = struct_item_type.as_ref().unwrap();
                                        let index_map = get_params_value_rs_struct(
                                            struct_item_type,
                                            &js_element
                                        );
                                        index_map.unwrap()
        ...
        .map(|(arg_type, val)| {
            match val {
                RsArgsValue::External(val) => {
                    Ok(
                        Box::into_raw(
                            Box::new(get_js_external_wrap_data(&env, val)?)
                        ) as *mut c_void
                    )
                }
                ...
                RsArgsValue::StructArray(val) => {
                    if let RsArgsValue::Object(arg_type) = arg_type {
                        ...
                        let FFIARRARYDESC { struct_item_type, array_len, .. } = array_desc;
                        let struct_item_type = struct_item_type
                            .as_ref()
                            .ok_or_else(||
                                FFIError::Panic("Missing struct item type".to_string())
                            )?;

                        let is_stack_struct =
                            get_ffi_tag(struct_item_type) == FFITypeTag::StackStruct;

                        if is_stack_struct {
                            let (struct_size, align) = calculate_struct_size(struct_item_type);
                            let mut head_ptr = None;
                            let mut current_ptr = alloc(
                                Layout::from_size_align(struct_size * array_len, align).unwrap()
                            ) as *mut c_void;

                            for item in val {
                                let struct_ptr = generate_c_struct(
                                    &env,
                                    struct_item_type,
                                    item,
                                    Some(current_ptr)
                    ...
                    if let RsArgsValue::Object(arg_type_rs) = arg_type {
                        ...
                        Ok(
                            if is_stack_struct {
                                ...
                                Box::into_raw(
                                    Box::new(generate_c_struct(&env, &arg_type_rs, val, None)?)
                                ) as *mut c_void
                            }
                        )
                    } else {
                ...
                RsArgsValue::Function(func_desc, js_function) => {
                    ...
                    let tsfn: ThreadsafeFunction<
                        Vec<RsArgsValue>,
                        ErrorStrategy::Fatal
                    > = (&js_function).create_threadsafe_function(
                        0,
                        move |ctx: ThreadSafeCallContext<Vec<RsArgsValue>>| {
                            let js_call_params: Vec<JsUnknown> = ctx.value
                                .into_iter()
                    ...
                    let (cif, lambda) = if
                        let RsArgsValue::Object(func_args_type_rs) = func_args_type
                    {
                        let cif = Cif::new(
                            func_args_type_rs
                                .values()
                        ...
                        let lambda = move |args: (Vec<*mut c_void>, *mut c_void)| {
                            ...
                            let value: Vec<RsArgsValue> = params
                                ...
                                .enumerate()
                                .map(|(index, c_param)| {
                                    let arg_type = func_args_type_rs
                                        .get(&index.to_string())
                                        .unwrap();
                                    let param = get_rs_value_from_pointer(
                                        env,
                                        arg_type,
                                        c_param,
                                        true
                                ...
                                func_ret_type != RsArgsValue::I32(7)
                            {
                                ...
                                (*tsfn_call_context_ptr).tsfn.call_with_return_value(
                                    ...
                                    move |js_return_value: JsUnknown| {
                                        let js_return_value_rs = get_arg_values(
                                ...
                                (*tsfn_call_context_ptr).tsfn.call(
                                    value,
                                    ThreadsafeFunctionCallMode::Blocking
                                );
                            }
                    ...
                    let code_ptr = std::mem::transmute(
                        (*tsfn_call_context_ptr).closure.as_ref().unwrap().code_ptr()
                    );
                    CLOSURE_MAP.as_mut()
                        .unwrap()
                ...
                RsArgsValue::Object(mut params_type_rs_value) => {
                        ...
                        )
                    {
                        ...
                        let array_value = match array_type {
                            RefDataType::U8Array => {
                                let js_buffer: JsBuffer = params_value_object.get_named_property(
                                    &field
                                )?;
                                RsArgsValue::U8Array(Some(js_buffer.into_value()?), None)
                            }
                            RefDataType::I16Array => {
                                let js_array: JsObject = params_value_object.get_named_property(
                                    &field
                                )?;
                            ...
                            RefDataType::StructArray => {
                                ...
                                let arg_val = vec![0; js_array.get_array_length()? as usize]
                                    ...
                                    .enumerate()
                                    .map(|(index, _)| {
                                        let js_element: JsObject = js_array
                                            .get_element(index as u32)
                                            .unwrap();
                                        let struct_item_type = struct_item_type.as_ref().unwrap();
                                        let index_map = get_params_value_rs_struct(
...
pub unsafe fn type_define_to_rs_args(env: &Env, type_define: JsUnknown) -> Result<RsArgsValue> {
    ...
    let ret_value = match params_type_value_type {
        ...
        ValueType::Object =>
            RsArgsValue::Object(
                type_object_to_rs_struct(env, &create_js_value_unchecked::<JsObject>(type_define)?)?
            ),
        _ => {
            return Err(
                FFIError::UnsupportedValueType(
                    format!("ret_value_type can only be number or object but receive {}", params_type_value_type)
                ).into()
            );
        }
    };
    ...
    match ret_type_rs {
        RsArgsValue::I32(number) => {
            ...
            match ret_data_type {
                ...
                BasicDataType::WString => {
                    let ptr_str = WideCString::from_ptr_str(
                        *(ptr as *mut *const WideChar)
                    ).to_string_lossy();
                    rs_value_to_js_unknown(&env, RsArgsValue::WString(ptr_str))
                }
                BasicDataType::U8 =>
                    rs_value_to_js_unknown(env, RsArgsValue::U8(*(ptr as *mut u8))),
                BasicDataType::I16 =>
                    rs_value_to_js_unknown(env, RsArgsValue::I16(*(ptr as *mut i16))),
                BasicDataType::I32 =>
                    rs_value_to_js_unknown(env, RsArgsValue::I32(*(ptr as *mut i32))),
                BasicDataType::I64 =>
                    rs_value_to_js_unknown(env, RsArgsValue::I64(*(ptr as *mut i64))),
                BasicDataType::U64 =>
                    rs_value_to_js_unknown(env, RsArgsValue::U64(*(ptr as *mut u64))),
                BasicDataType::BigInt => {
                ...
                BasicDataType::Float =>
                    rs_value_to_js_unknown(env, RsArgsValue::Float(*(ptr as *mut f32))),
                BasicDataType::Double => {
        ...
        RsArgsValue::Object(sub_obj_type) => {
            if let FFITypeTag::Array | FFITypeTag::StackArray = get_ffi_tag(&sub_obj_type) {
                ...
                // array
                let FFIARRARYDESC { array_type, array_len, struct_item_type, .. } = array_desc;
                match array_type {
                    RefDataType::U8Array => {
                    ...
                    RefDataType::StructArray => {
                        ...
                        let is_stack_struct =
                            get_ffi_tag(struct_item_type.as_ref().unwrap()) ==
                            FFITypeTag::StackStruct;
                        let v = (0..array_len)
                            .map(|_| {
                                ...
                                let (struct_size, _) = calculate_struct_size(
                                    &struct_item_type.as_ref().unwrap()
                                );
                                if is_stack_struct {
    ...
    match &ret_type {
        RsArgsValue::I32(number) => {
            ...
            match ret_data_type {
                ...
                BasicDataType::String =>
                    std::ptr::copy(src, dst, std::mem::size_of::<*const c_char>()),
                BasicDataType::WString =>
                    std::ptr::copy(src, dst, std::mem::size_of::<*const WideChar>()),
                BasicDataType::External =>
                    std::ptr::copy(src, dst, std::mem::size_of::<*mut c_void>()),
                BasicDataType::Void => {}
            }
            match ret_data_type {
                | BasicDataType::U8
