        ...
        RsArgsValue::I32(number) => {
            let data = match (*number).try_into().unwrap() {
                ...
                BasicDataType::WString => {
                    RsArgsValue::WString(
                        create_c_w_string_from_ptr(*(pointer as *mut *mut WideChar))
                    )
                }
