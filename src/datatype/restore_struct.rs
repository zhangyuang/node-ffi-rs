        ...
        if let RsArgsValue::I32(number) = val {
            ...
            if let FFITypeTag::Array | FFITypeTag::StackArray = get_ffi_tag(sub_obj_type) {
                ...
                match array_type {
                    ...
                    RefDataType::DoubleArray => {
                        ...
                        if dynamic_array {
                            ...
                            let arr = create_static_array_from_pointer(
                    ...
                    RefDataType::U8Array => {
                        ...
                        if dynamic_array {
                            ...
                            if let RsArgsValue::U8Array(_, arr) = arr {
                                rs_struct.insert(
                                    field,
                                    get_safe_buffer(env, arr.unwrap(), need_thread_safe)
                                );
                            }
                        }
                        offset += size + padding;
                ...
                if get_ffi_tag(&sub_obj_type) == FFITypeTag::StackStruct {
                    ...
                    let padding = (align - (offset % align)) % align;
                    field_ptr = field_ptr.offset(padding as isize);
                    let sub_object = RsArgsValue::Object(
                        create_rs_struct_from_pointer(
                            env,
                            field_ptr,
                            sub_obj_type,
                            need_thread_safe
...
pub fn create_js_object_from_rs_map(
    ...
    for (field, value) in rs_struct {
        js_object
            .set_property(env.create_string(&field)?, rs_value_to_js_unknown(&env, value)?)
            .unwrap();
    }
    Ok(js_object)
}
pub fn rs_value_to_js_unknown(env: &Env, data: RsArgsValue) -> Result<JsUnknown> {
    let res = match data {
        ...
        RsArgsValue::String(val) | RsArgsValue::WString(val) =>
            env.create_string(&val)?.into_unknown(),
